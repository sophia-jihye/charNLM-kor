...
def LSTMCNN(char_vocab_size, char_vec_size, feature_maps, kernels, batch_size, seq_length, max_word_l, batch_norm, highway_layers, num_layers, rnn_size, dropout, word_vocab_size, learning_rate, max_grad_norm):
    ...
    for l in range(num_layers):
        x = LSTM(rnn_size, activation='tanh', recurrent_activation='sigmoid', return_sequences=True, stateful=True)(x)

        if dropout > 0:
            x = Dropout(dropout)(x)

    output = TimeDistributed(Dense(word_vocab_size, activation='softmax'))(x)

    model = sModel(inputs=inputs, outputs=output)
    model.summary()

    optimizer = sSGD(lr=learning_rate, clipnorm=max_grad_norm, scale=float(seq_length))
